...
def generate_multiline_text(
    text: str, max_width: int, font: ImageFont.ImageFont, spacing: int
):
    background_draw = ImageDraw.Draw(
        Image.new("RGB", (BACKGROUND_WIDTH, BACKGROUND_WIDTH))
    )
    words = text.split(" ")
    ...
    for word in words[1:]:
        temp_width, _ = background_draw.multiline_textsize(
            f"{current_line_text} {word}", font=font, spacing=spacing
        )
...
class DemotivateCommand(Command):
    ...
    async def send_image(self, message: types.Message):
        ...
        multiline_title_text = generate_multiline_text(
            title_text, max_text_width, TITLE_TEXT_FONT, TEXT_SPACING
        )
        multiline_lower_text = generate_multiline_text(
            lower_text, max_text_width, LOWER_TEXT_FONT, TEXT_SPACING // 1.7
        )
        ...
        calculated_background_height = (
            2 * PADDING_Y
            + 2 * BORDER_PADDING
            + image.height
            + len(multiline_title_text) * TITLE_TEXT_SPACING
            - TEXT_SPACING
            + (
                len(multiline_lower_text) * LOWER_TEXT_SPACING - TEXT_SPACING // 1.7
                if lower_text
                else 0
            )
        )
        ...
        background = Image.new(
            "RGB", (BACKGROUND_WIDTH, int(calculated_background_height))
        )
        background_draw = ImageDraw.Draw(background)

        background.paste(
            image,
            (BACKGROUND_WIDTH // 2 - image_width // 2, PADDING_Y + BORDER_PADDING),
        )
        background_draw.rectangle(
            (
                BACKGROUND_WIDTH // 2 - image_width // 2 - BORDER_PADDING,
                PADDING_Y,
                BACKGROUND_WIDTH // 2 + image_width // 2 + BORDER_PADDING,
                PADDING_Y + image_height + 2 * BORDER_PADDING,
            ),
            outline=(255, 255, 255),
            width=4,
        )
        ...
        for i, title_text_line in enumerate(multiline_title_text):
            background_draw.text(
                (BACKGROUND_WIDTH // 2, start_text_position + TITLE_TEXT_SPACING * i),
                title_text_line,
                fill=(255, 255, 255),
            ...
            for i, lower_text_line in enumerate(multiline_lower_text):
                background_draw.text(
                    (
                        BACKGROUND_WIDTH // 2,
                        start_text_position
                        + TITLE_TEXT_SPACING * len(multiline_title_text)
                        + LOWER_TEXT_SPACING * i,
                    ),
                    lower_text_line,
                    fill=(255, 255, 255),
